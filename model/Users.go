...
// 参数: query - GORM 查询对象, currentRole - 当前用户角色
// 返回: 应用过滤后的查询对象
func applyRoleFilter(query *gorm.DB, currentRole int) *gorm.DB {
	switch currentRole {
	case 2:
...
func SearchUser(keyword string, role int, pageSize int, pageNum int, currentRole int) ([]User, int64) {
	...
	// 构建查询条件（使用公共的权限过滤函数）
	query := applyRoleFilter(db, currentRole)
	...
	if pageSize == -1 || pageNum == -1 {
		...
		err = query.Limit(pageSize).Offset((pageNum - 1) * pageSize).Find(&users).Error
	}
...
func EditUser(id int, data *User) int {
	...
	if data.Password != "" {
		...
		// 所以这里需要手动加密
		encryptedPw, encryptErr := EncryptPassword(data.Password)
		if encryptErr != nil {
			return errmsg.ERROR
		}
